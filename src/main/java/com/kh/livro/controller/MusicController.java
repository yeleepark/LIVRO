...
public class MusicController {
	...
	*/
	...
	public String fileUpload(HttpServletRequest request, Model model, MemberDto memberdto, MusicDto musicDto, BindingResult result) {
		...
		try {
			inputStream = file.getInputStream(); //업로드된 파일의 입력스트림을 변수에 저장
			String path = WebUtils.getRealPath(request.getSession().getServletContext() , "/resources/storage");
			//프로젝트 내부에 파일 업로드하는 경로  //현재 사용중인 프로젝트 경로가 어디인가
			...
			File newFile = new File(path+"/"+name);	//경로가 존재할 경우 경로명에 /파일명 을 붙여 객체 생성
